 ...
 * Generated on                   : Fri Jul 26 13:15:47 2024
 */
...
PIL_CONFIG_DEF(uint32_t, ExtMode_targetFloat_Size,
               sizeof(pl_28379D_FloatType));
PIL_CONFIG_DEF(uint32_t, ExtMode_targetPointer_Size,
               sizeof(pl_28379D_FloatType*));
PIL_CONFIG_DEF(uint32_t, ExtMode_sampleTime_Ptr,
               (uint32_t)&pl_28379D_sampleTime);
PIL_CONFIG_DEF(uint32_t, ExtMode_checksum_Ptr, (uint32_t)&pl_28379D_checksum);
#if defined(pl_28379D_NumTunableParameters) && \
   (pl_28379D_NumTunableParameters > 0)
PIL_CONFIG_DEF(uint32_t, ExtMode_P_Ptr, (uint32_t)&pl_28379D_P);
PIL_CONFIG_DEF(uint32_t, ExtMode_P_Size,
               (uint32_t)pl_28379D_NumTunableParameters);
#endif
#if defined(pl_28379D_NumExtModeSignals) && (pl_28379D_NumExtModeSignals > 0)
PIL_CONFIG_DEF(uint32_t, ExtMode_ExtModeSignals_Ptr,
               (uint32_t)&pl_28379D_ExtModeSignals[0]);
PIL_CONFIG_DEF(uint32_t, ExtMode_ExtModeSignals_Size,
               (uint32_t)pl_28379D_NumExtModeSignals);
#endif

#define CODE_GUID {0x09, 0x3c, 0x97, 0x5e, 0xdb, 0x2d, 0xb8, 0x40};
PIL_CONST_DEF(unsigned char, Guid[], CODE_GUID);
PIL_CONST_DEF(unsigned char, CompiledDate[], "07/26/2024 01:15 PM");
...
uint16_t ScopeBuffer[1052] /*__attribute__((aligned(16)))*/;
...
extern void pl_28379D_step();
extern void pl_28379D_enableTasksInterrupt();
extern void pl_28379D_syncTimers();
static void Tasks(bool aInit, void * const aParam)
{
   if(aInit)
   {
      pl_28379D_enableTasksInterrupt();
   }
   else
   {
      pl_28379D_step();
...
interrupt void pl_28379D_baseTaskInterrupt(void)
{
...
void pl_28379D_enableTasksInterrupt(void)
{
   IER |= M_INT1;
}

/* Timer Synchronization Code */
void pl_28379D_syncTimers(void)
{
   CpuTimer0Regs.TCR.bit.TSS = 0;
}

/* Background tasks */
void pl_28379D_background(void)
{

}
/* HAL Initialization Code */
static bool HalInitialized = false;
void pl_28379D_initHal()
{
   ...
   PIL_setChecksum(PilHandle, pl_28379D_checksum);
   PIL_setAndConfigScopeBuffer(PilHandle, (uint16_t *)&ScopeBuffer, 1052, 26);
   ...
   DISPR_registerIdleTask(&pl_28379D_background);
   DISPR_registerSyncCallback(&pl_28379D_syncTimers);
   ...
   *(PINT *)((uint32_t)(&PieVectTable.ADCA1_INT) + ((uint32_t)0)*
             sizeof(PINT *)) = &pl_28379D_baseTaskInterrupt;
